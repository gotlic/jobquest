...
`;function L(e,t,T){try{let E=e.exec(`PRAGMA table_info(${t})`);if(!E[0])return!1;let a=E[0].columns.indexOf("name");return E[0].values.some(e=>e[a]===T)}catch{return!1}}function l(e,t){try{let T=e.exec(`SELECT name FROM sqlite_master WHERE type='table' AND name='${t}'`);return!!T[0]?.values?.length}catch{return!1}}let o=null,c=null,p=0;async function U(){return o||(o=await (0,t.default)()),o}async function _(){let e,t=0;try{t=T.default.statSync(i).mtimeMs}catch{}if(c&&t===p)return c;let E=await U();if(T.default.existsSync(i)){let t=T.default.readFileSync(i);e=new E.Database(t)}else e=new E.Database;e.exec(d);var a=e;a.exec("CREATE TABLE IF NOT EXISTS migrations (name TEXT PRIMARY KEY, applied_at TEXT NOT NULL DEFAULT (datetime('now')))");let s=new Set;try{let e=a.exec("SELECT name FROM migrations");e[0]?.values.forEach(e=>s.add(e[0]))}catch{}if(!s.has("spaces_v1")){for(let e of(l(a,"spaces")||a.exec(`CREATE TABLE spaces (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        slug TEXT NOT NULL UNIQUE,
        name TEXT NOT NULL,
        password_hash TEXT NOT NULL,
        ...
        settings TEXT NOT NULL DEFAULT '{}',
        created_at TEXT NOT NULL DEFAULT (datetime('now'))
      )`),["jobs","cvs","cv_categories","cover_letters"]))if(l(a,e)&&!L(a,e,"space_id"))try{a.exec(`ALTER TABLE ${e} ADD COLUMN space_id INTEGER NOT NULL DEFAULT 1`)}catch{}l(a,"feed_blocklist")&&!L(a,"feed_blocklist","space_id")&&a.exec(`
      ...
      `);let e=r("lic12@"),t=r("tomot123");a.exec(`INSERT OR IGNORE INTO spaces (id, slug, name, password_hash) VALUES (1, 'victor', 'Victor', '${e}')`),a.exec(`INSERT OR IGNORE INTO spaces (id, slug, name, password_hash) VALUES (2, 'tom', 'Tom', '${t}')`),a.exec("INSERT OR IGNORE INTO migrations (name) VALUES ('spaces_v1')")}return T.default.writeFileSync(i,Buffer.from(e.export())),p=T.default.statSync(i).mtimeMs,c=new N(e,i)}e.s(["getDb",0,_],62294)},88942,e=>{"use strict";var t=e.i(37709);e.s(["default",()=>t.Anthropic])},6714,e=>{e.v(t=>Promise.all(["server/chunks/[externals]_node_fs_1t1l-4-._.js"].map(t=>e.l(t))).then(()=>t(2157)))},11105,e=>{e.v(t=>Promise.all(["server/chunks/[externals]_node_path_1pmhwj3._.js"].map(t=>e.l(t))).then(()=>t(50227)))},46735,e=>{e.v(t=>Promise.all(["server/chunks/[externals]__1j5vgk-._.js","server/chunks/[root-of-the-server]__1fbyaci._.js"].map(t=>e.l(t))).then(()=>t(83085)))}];

//# sourceMappingURL=%5Broot-of-the-server%5D__0wuu5ss._.js.map